...
import com.payment.core.entity.CurrencyAccount;
...
public class SolController {
    ...
    public Result<RechargeResponse> createRechargeRecord(@PathVariable String orderId, @PathVariable String amount) {
        ...
        // TODO create sol address
        SolAddress solAddress = new SolAddress();
        recharge.setToAddress(solAddress.getAddress());
        ...
        account.setAddress(solAddress.getAddress());
        account.setCurrencyId(ethInfo.getId());
        account.setCurrencyName(ethInfo.getCurrencyName());
        account.setKeystoreName(solAddress.getKeystoreName());
        account.setMnemonic(solAddress.getMnemonic());
        Date now = new Date();
        ...
        //二维码数据拼接
        String qrcodeData = "sol:" + solAddress.getAddress() + "?value=" + amount;

        return Result.buildSuccess(new RechargeResponse(solAddress.getAddress(), qrcodeData));
    }
}
